...
namespace online_course_recommendation_system.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Login([FromBody] LoginDto request)
        {
            ...
            Console.WriteLine($"[DEBUG-LOGIN] Email: '{request.Email}', Password Length: {request.MatKhau?.Length}");
            ...
            if (user == null || user.MatKhau != HashPassword(request.MatKhau))
            {
                if (user != null)
                {
                    Console.WriteLine($"[DEBUG-LOGIN] FAILED: Hash mismatch. Input hash: {HashPassword(request.MatKhau)}, DB hash: {user.MatKhau}");
                }
